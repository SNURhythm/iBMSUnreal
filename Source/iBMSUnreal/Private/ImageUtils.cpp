...
void ImageUtils::LoadTexture2D(const FString ImageFileName, const TArray<uint8>& RawFileData, bool& IsValid,
                               int32 TargetWidth, int32 TargetHeight, UTexture2D*& OutTexture, bool UpdateResource)
{
	IsValid = false;
	...
	TSharedPtr<IImageWrapper> ImageWrapper = ImageWrapperModule.CreateImageWrapper(
		ImageFileName.EndsWith(".png")
			? EImageFormat::PNG
			: (ImageFileName.EndsWith(".bmp") ? EImageFormat::BMP : EImageFormat::JPEG));
		...
		if (ImageWrapper->GetRaw(ERGBFormat::BGRA, 8, UncompressedBGRA))
		{
			...
			if (OutTexture != nullptr)
			{
				...
				for (int32 y = 0; y < TargetHeight; y++)
				{
					...
					{
						int32 srcIndex = ((y * CropHeight) / TargetHeight * CropWidth + (x * CropWidth) / TargetWidth) *
							4;
