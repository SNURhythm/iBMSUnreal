						...
						if (LongNote->IsTail())
						{
							...
							if (Options.AutoPlay)
							{
								Renderer->OnLaneReleased(
									Note->Lane, std::chrono::duration_cast<std::chrono::microseconds>(
...
void ARhythmControl::OnMediaOpened(FString OpenedUrl)
{
	...
	UGameplayStatics::ProjectWorldToScreen(GetWorld()->GetFirstPlayerController(), RightBottomWorld,
	                                       RightBottomProjected);
...
{
	if (Note->Wav != FBMSParser::NoWav && !Options.AutoKeysound)
	{
		Jukebox->PlayKeySound(Note->Wav);
	}
...
int ARhythmControl::PressLane(int MainLane, int CompensateLane, double InputDelay)
{
	...
	if (State == nullptr)
	{
		...
		Renderer->OnLanePressed(MainLane, FJudgeResult(None, 0),
	...
	for (int i = State->PassedMeasureCount; i < Measures.Num(); i++)
	{
		...
		for (int j = IsFirstMeasure ? State->PassedTimelineCount : 0; j < Measure->TimeLines.Num(); j++)
		{
			...
			for (auto Lane : AvailableCandidates)
			{
				...
				Renderer->OnLanePressed(Lane, Judgement,
				                        std::chrono::duration_cast<std::chrono::microseconds>(
					                        std::chrono::system_clock::now().time_since_epoch()).count());
				return Lane;
			}
...
void ARhythmControl::ReleaseLane(int Lane, double InputDelay)
{
	...
	Renderer->OnLaneReleased(
		Lane, std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now().time_since_epoch())
		.count());
...
void ARhythmControl::BeginPlay()
{
	...
	// on esc
	FInputKeyBinding Press(FInputChord(EKeys::Escape, false, false, false, false), IE_Pressed);
...
void ARhythmControl::LoadGame()
{
	...
	if (!Options.AutoPlay)
	{
		...
		bool result1 = InputHandler->StartListenNative() || InputHandler->StartListenUnreal(PlayerInputComponent);
		// fallback to unreal input if native input failed
